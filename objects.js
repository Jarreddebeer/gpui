...
export class Object3d {
    ...
    constructor(mesh) {
        ...
        this.color       = Theme.col1;
        this._pivot      = new XYZ(0, 0, 0);
    ...
    set interactive(v) {
        ...
        if (v && this.id == null) {
            ...
            this.interactiveMesh = this.mesh.copyShallow();
            this.interactiveMesh.col = this.mesh.col.copy();
            this.interactiveMesh.color = new RGBA(
                ((this.id >>  0) & 0xFF) / 0xFF,
                ((this.id >>  8) & 0xFF) / 0xFF,
                ((this.id >> 16) & 0xFF) / 0xFF,
                ((this.id >> 24) & 0xFF) / 0xFF
            );
            interactiveObjects[this.id] = this;
        }
    }
    
    static getInteractiveObject(id) {
    ...
    set color(rgba) { 
        this._color = rgba;
        if (this.mesh) this.mesh.color = rgba; 
    }

    set pivot(xyz) {
        this.mesh.offset = new XYZ(this._pivot.x, this._pivot.y, this._pivot.z);
        this._pivot = xyz;
    ...
    bake() {
        const translate = vec3.clone(this.translate.xyz.vec);
        vec3.add(translate, translate, this._pivot.vec);
        mat4.fromTranslation(this.transform, translate);
...
export class Camera {
    ...
    constructor(gl, width, height) {
        this.gl         = gl;
        this._width     = gl.drawingBufferWidth || width;
        this._height    = gl.drawingBufferHeight || height;
        this.near       = 0.01;
        this.far        = 1000;
    ...
    get width()  { return ((this.gl) ? this.gl.drawingBufferWidth : this._width); }
    
    set height(v) { this._height = v; }
    get height()  { return ((this.gl) ? this.gl.drawingBufferHeight : this._height); }
...
export class Cleanser {
    ...
    static get objects() { return _cleanser_objects; }
    static get attributes() { return _cleanser_attributes; }
    static get primitives() { return _cleanser_primitives; }
    ...
    static processObject(object) {
        ...
        for (let primitive of Cleanser.primitives) {
            if ((object.translate && (object.translate._xyz == primitive || object.translate._offset == primitive)) ||
                (object.rotate &&    (object.rotate._xyz    == primitive || object.rotate._offset    == primitive)) ||
                (object.scale &&     (object.scale._xyz     == primitive || object.scale._offset     == primitive)) ||
                (object.color &&     (object.color          == primitive))) {
                    Cleanser.objects.add(object);
                    return;
            }
        }
