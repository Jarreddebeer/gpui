import { Quad2d, Quad3d, XYZ } from "./objects";
...
export class Mesh {
    ...
    constructor(length) {
        if (!isNaN(length)) {
            length = length ? Math.max(0, length) : 0;
            // TODO: note, this is currenly only configured for quads
    ...
    setQuad(quad, rgb, i) {
        ...
        if (rgb) this.setQuadCol(colData, i);
        this.setQuadInd(i);
    }
